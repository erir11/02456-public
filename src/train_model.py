...
class _single_task_segmentation(_segmentation_model):
    ...
    def test_step(self, batch, batch_idx):
        ...
        pred_iou[pred_iou > 8] = 0
        self.test_step_outputs.append((pred_iou.squeeze(), y.squeeze()))
        # examples = []
        # for x, targ, p in zip(x, y.unsqueeze(0), pred):
        #     _, _, iou = mean_iou([p],[targ], ignore_index=-1, num_classes=self.num_classes)
        #     name = plot_eval(img=x.cpu().squeeze(), mask=targ.cpu().squeeze(), pred=p.cpu().squeeze(), classes=self.num_classes, iou=iou)
        #     image = wandb.Image(name, caption=name)
    ...
    def on_test_epoch_end(self):
        ...
        classes = ["Hood", "Front door", "Rear door", "Frame", "Rear quarter panel", "Trunk lid"]
        ...
        self.log("mIoU "+"Bumper", iou[8], on_step=False, on_epoch=True, prog_bar=True, logger=True)
        self.log("dice "+"Bumper", dice[8], on_step=False, on_epoch=True, prog_bar=True, logger=True)
...
class segformer(_single_task_segmentation):
